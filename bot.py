...
# Configuration for Imgflip memes (for custom "Edit Meme" mode)
MEME_TEMPLATES = {
...
# Reddit request headers (update the username as needed)
REDDIT_HEADERS = {
...
def generate_custom_meme(category: str, top_text: str, bottom_text: str) -> str:
    """Generate custom meme using Imgflip API."""
...
def fetch_reddit_video(category: str) -> str:
    ...
    # Mapping for video subreddits for video mode (only for "dark humor" and "distracted")
    subreddit_map = {
    ...
    for subreddit in subreddit_map.get(category, []):
        try:
            ...
            for post in posts:
                data = post.get('data', {})
                # For video posts, check if 'is_video' is True and URL ends with .mp4
...
def fetch_random_meme(mode: str, category: str = None) -> str:
    """Fetch random meme using Reddit API. For 'random' mode, use r/memes; for 'video' mode, use fetch_reddit_video."""
    if mode == "random":
...
def handle_option(update: Update, context: CallbackContext) -> int:
    ...
    # For video mode, show only VIDEO_CATEGORIES; otherwise, show ALL_CATEGORIES.
...
def main():
    TOKEN = "7888148153:AAEcuNamNUHQR0aBJemNpL73HCS-nBwZ4gc"  # Your Telegram bot token
